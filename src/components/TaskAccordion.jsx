  ...
  return (
    <div className="h-full min-h-0 w-full flex flex-col lg:flex-row gap-3 lg:gap-6">
        ...
        {tasks.map((task, index) => {
          ...
          return (
            <button
              ...
              className={`snap-start shrink-0 flex items-center gap-2 px-4 py-2 rounded-full text-[11px] font-bold uppercase tracking-[0.2em] border transition-all duration-200 ${
                isActive
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                  submitted ? "bg-green-500" : isActive ? "bg-[#ff4444]" : "bg-white/20"
                }`}
              />
              Task {index + 1}
              {submitted && <CheckCircle2 className="w-3 h-3 text-green-500 shrink-0" />}
            </button>
          );
        })}
      </div>

      {/* ── DESKTOP: tall sidebar cards ── */}
      ...
      <div className="flex-1 min-h-0 min-w-0 flex flex-col bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl relative font-sans" style={{ overflow: "hidden" }}>
        ...
        {/* Header — outside scroll container so content never bleeds through */}
        ...
        >
          {/* Body */}
          <div className="px-5 sm:px-10 lg:px-12 py-5 sm:py-10 space-y-8 sm:space-y-10">
                ...
                <ul className="space-y-3 sm:space-y-4">
                  {(() => {
                    ...
                    return activeTask.instructions.map((inst, i) => {
                      ...
                      return (
                        <li key={i} className="flex items-start gap-3 sm:gap-4 text-white/80">
        ...
        {/* Footer */}
        <div
          className={`shrink-0 px-5 sm:px-10 lg:px-12 py-4 sm:py-5 border-t border-white/5 bg-black/40 flex ${activeTask.containsAssets ? "justify-between" : "justify-end"} items-center relative z-10`}
        >
          {activeTask.containsAssets && (
            <button
              onClick={handleDownloadAllAssets}
              disabled={isDownloading}
              className="flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3.5 bg-white/5 hover:bg-white/10 text-white rounded-full font-bold uppercase tracking-widest text-xs transition-colors border border-white/10"
            >
              {isDownloading ? (
          ...
          {isSubmitted ? (
            <button
              disabled
              className="flex items-center justify-center gap-2 px-5 sm:px-8 py-2.5 sm:py-3.5 bg-green-500/10 text-green-500 rounded-full font-bold uppercase tracking-widest text-xs border border-green-500/20 cursor-not-allowed opacity-80"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-5 sm:px-8 py-2.5 sm:py-3.5 bg-[#9b1a1a] hover:bg-[#b01e1e] text-white rounded-full font-bold uppercase tracking-widest text-xs transition-colors shadow-[0_0_16px_rgba(155,26,26,0.3)] hover:shadow-[0_0_28px_rgba(155,26,26,0.5)] border border-[#ff3333]/20"
            >
