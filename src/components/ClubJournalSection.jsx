...
function JournalText({ date, title, text, align }) {
  return (
    <div 
      className="aspect-square flex flex-col justify-center p-6 md:p-10 border border-black/10"
      style={light}
    >
      <div className={`flex flex-col ${align === "right" ? "items-end text-right" : "items-start text-left"}`}>
        <span className="text-[#9b1a1a] text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] mb-2 md:mb-4">{date}</span>
        <h3 className="text-[#0a0a0a] text-lg md:text-2xl lg:text-3xl font-normal uppercase tracking-normal mb-2 md:mb-4 leading-none">{title}</h3>
        <p className="text-[#0a0a0a]/70 text-[10px] md:text-xs leading-relaxed font-bold">{text}</p>
      </div>
    </div>
  );
}
