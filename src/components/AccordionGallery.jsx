  ...
  const applyLayout = useCallback(animate => {
    ...
    panels.forEach((panel, i) => {
      ...
      if (showLabels && bar && text) {
        const elementsToAnimate = [bar, text];
        if (link) elementsToAnimate.push(link);
        ...
        if (isActive) {
          ...
          tl.to(elementsToAnimate, { opacity: 0, x: -14, duration: dur * 0.6, ease }, 0);
        }
      }
    });
            ...
            {showLabels && (
              <span
                ...
                <span
                  ...
                  className="overflow-hidden text-ellipsis whitespace-nowrap text-[clamp(1rem,1.4vw,1.4rem)] font-semibold tracking-[0.01em] opacity-0 [text-shadow:0_2px_14px_rgba(0,0,0,0.55)] flex-1"
