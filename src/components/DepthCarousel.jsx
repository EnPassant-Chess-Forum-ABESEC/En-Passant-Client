...
const DepthCarousel = ({
  ...
  const handleCardClick = useCallback(index => {
    ...
    setFocus(index, true);
    if (onCardClick) {
      onCardClick(index, data[index]);
    }
  }, [setFocus, onCardClick, data]);
  ...
  return (
    <div
      ref={rootRef}
      className={`relative flex h-full min-h-[320px] w-full select-none items-center justify-center outline-none [perspective-origin:50%_50%] focus-visible:rounded-xl focus-visible:outline-2 focus-visible:outline-white/50 focus-visible:[outline-offset:4px] ${className}`.trim()}
          ...
          {data.map((_, i) => (
            <button
              ...
              className={`h-[7px] cursor-pointer rounded-full transition-all duration-[250ms] ${
                active === i ? 'w-[7px] bg-white scale-125 shadow-[0_0_8px_rgba(255,255,255,0.8)]' : 'w-[7px] bg-white/30'
              }`}
